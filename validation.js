...
export function goalDateChecker(date){
	...
	if (
		date.substring(0, 2) === '04' ||
		date.substring(0, 2) === '06' ||
		date.substring(0, 2) === '09' ||
		date.substring(0, 2) === '11'
	) {
		if (parseInt(date.substring(3, 5)) > 30 || parseInt(date.substring(3, 5)) < 0)
			throw `Invalid day`;
	} else if (
		date.substring(0, 2) === '01' ||
		date.substring(0, 2) === '03' ||
		date.substring(0, 2) === '05' ||
		date.substring(0, 2) === '07' ||
		date.substring(0, 2) === '08' ||
		date.substring(0, 2) === '10' ||
		date.substring(0, 2) === '12'
	) {
		if (parseInt(date.substring(3, 5)) > 31 || parseInt(date.substring(3, 5)) < 0)
			throw `Invalid day`;
	} else {
		if (parseInt(date.substring(3, 5)) > 28 || parseInt(date.substring(3, 5)) < 0)
			throw `Invalid day`;
	}
	...
	if (year === goalYear && month > goalMonth)
		throw `Goal Date must be a future date (invalid month)`;
	if (year === goalYear && month === goalMonth && day > goalDay)
		throw `Goal Date must be a future date (invalid day)`;
...
export function dateWithin7Days(date) {
	//todays date is false, since goal may not yet be met
	if (date[2] != '/' && date[5] != '/')
...
export function dateIsInThePast(date) {
	//todays date is false, goals with this date not in past
	if (date[2] != '/' && date[5] != '/')
		throw `Release date needs to be in the format 'MM/DD/YYYY'`;
	let validMonths = ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12'];
	let present = false;
