...
function GoalCard(props) {
	...
	const deleteExpense = async (expenseId, goalId) => {
		try {
			...
			let deletedExpense = await axios.delete(
				`http://54.175.184.234:3000//user/${uid}/${goalId}/${expenseId}`
			);
	...
	async function deleteGoal(goalId) {
		let id = doGetUID();
		let data = await axios.delete(`http://54.175.184.234:3000//userProfile/${id}/${goalId}`);
		handleClose();
		setShowDeleteGoalForm(false);
	}
	...
	useEffect(() => {
		...
		async function getGoalInfo() {
			let id = doGetUID();
			try {
				let data = await axios.get(`http://54.175.184.234:3000//userProfile/${id}/${props.id}`);
				let goal = data.data;
				setGoal(goal);
